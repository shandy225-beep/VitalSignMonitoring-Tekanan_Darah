...
class Pan_Tompkins_Plus_Plus():
    ...
    def rpeak_detection(self, ecg, fs):
        ...
        # Temukan semua kandidat puncak (pakai threshold rendah dulu)
...
def _get_window(kernel, size, **kwargs):
    try:
        return get_window(kernel, size, **kwargs)
    except Exception as e:
        raise ValueError(f"Unknown or invalid window: {kernel}. Error: {e}")
    return window
